...
namespace Slic3r {
std::optional<boost::filesystem::path> get_home_config_dir() { return get_home_dir("/.config"); }

std::optional<boost::filesystem::path> get_home_local_dir() { return get_home_dir("/.local"); }
} // namespace Slic3r
...
std::string get_default_datadir() {
    ...
    std::string data_dir =
        (boost::filesystem::path(config_dir) / SLIC3R_APP_FULL_NAME).make_preferred().string();
    return data_dir;
}
