///|/ Copyright (c) Prusa Research 2020 - 2023 Vojtěch Bubník @bubnikv, Lukáš Hejl @hejllukas, Lukáš
/// Matěna @lukasmatena, Roman Beránek @zavorka
///|/
...
#ifdef _WIN32
// The new API is better than the old SEH style thread naming since the names also show up in crash
// dumpsand ETW traces. Because the new API is only available on newer Windows 10, look it up dynamically.
...
static bool WindowsGetSetThreadNameAPIInitialize() {
    if (!s_SetGetThreadDescriptionInitialized) {
        // Not thread safe! It is therefore a good idea to name the main thread before spawning
        // worker threads to initialize
        s_hKernel32 = LoadLibraryW(L"Kernel32.dll");
        if (s_hKernel32) {
            s_fnSetThreadDescription = (SetThreadDescriptionType
            )::GetProcAddress(s_hKernel32, "SetThreadDescription");
            s_fnGetThreadDescription = (GetThreadDescriptionType
            )::GetProcAddress(s_hKernel32, "GetThreadDescription");
        }
...
#ifndef NDEBUG
// Use the old way by throwing an exception, so at least in Debug mode the thread names are shown by
// the debugger.
...
static void WindowsSetThreadNameSEH(HANDLE hThread, const char *thread_name) {
    ...
    __try {
        RaiseException(
            MSVC_SEH_EXCEPTION_NAME_THREAD, 0, sizeof(info) / sizeof(ULONG_PTR), (ULONG_PTR *) &info
        );
...
ThreadData &thread_data() { return s_thread_data; }

// Spawn (n - 1) worker threads on Intel TBB thread pool and name them by an index and a system
// thread ID. Also it sets locale of the worker threads to "C" for the G-code generator to produce
...
void name_tbb_thread_pool_threads_set_locale() {
    ...
    // TBB will respect the task affinity mask on Linux and spawn less threads than
    ...
    tbb::parallel_for(
        ...
        [&nthreads_running, nthreads, &master_thread_id, &cv,
         &cv_m](const tbb::blocked_range<size_t> &range) {
...
#ifdef __APPLE__
    // OSX specific: Set Quality of Service to "user initiated", so that the threads will be
    // scheduled to high performance cores if available. With QOS_CLASS_USER_INITIATED the worker
    // threads drop priority once slicer loses user focus.
...
void ThreadData::tbb_worker_thread_set_c_locales() {
    ...
    //    std::cout << "TBBLocalesSetter Entering " << cnt ++ << " ID " <<
